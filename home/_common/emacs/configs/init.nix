...
with lib; let
  ...
  usePackageType = types.submodule ({
    ...
    options = {
      ...
      custom = mkOption {
        type = with types; let
          primitive = oneOf [bool int float str];
        in
      ...
      assembly = let
        ...
        mkCustomHelper = mapAttrsToList (n: v: let
          mkValue = v:
            ...
            then v
            else throw "Unsupported custom value: ${toString v}";
        in "(${n} ${mkValue v})");
        mkCustom = cs:
  ...
  gcSettings = ''
    ...
    (phd/reduce-gc)
    (add-hook 'minibuffer-setup-hook #'phd/reduce-gc -50)
    (add-hook 'kill-emacs-hook #'phd/reduce-gc -50)

    ;; But make it more regular after startup and after closing minibuffer.
    (add-hook 'emacs-startup-hook #'phd/restore-gc)
    (add-hook 'minibuffer-exit-hook #'phd/restore-gc)

    ;; Avoid unnecessary regexp matching while loading .el files.
    (defvar phd/file-name-handler-alist file-name-handler-alist)
    (setq file-name-handler-alist nil)

    (defun phd/restore-file-name-handler-alist ()
      "Restores the file-name-handler-alist variable."
      (setq file-name-handler-alist phd/file-name-handler-alist)
      (makunbound 'phd/file-name-handler-alist))

    (add-hook 'emacs-startup-hook #'phd/restore-file-name-handler-alist)
  '';
  ...
  earlyInitFile = ''
    ;;; hm-early-init.el --- Emacs configuration via Home Manager -*- lexical-binding: t; -*-
    ;;
    ;;; Commentary:
    ;;
  ...
  in
    ''
      ...
      ;;; Commentary:
      ;;
      ;; A configuration generated from a Nix based configuration by Home Manager.
      ;;
      ;;; Code:

      ${optionalString cfg.startupTimer ''
        (defun phd/print-startup-stats ()
        ...
        (add-hook 'emacs-startup-hook #'phd/print-startup-stats)
      ''}
...
in {
  ...
  options.programs.emacs.init = {
    ...
    globalBinds = mkOption {
      ...
      default = {};
      description = ''
        Configuration set describing global keymap binds in
        <filename>init.el</filename>.
      '';
    };
  ...
  config = mkIf (config.programs.emacs.enable && cfg.enable) {
    ...
    home.file = {
      ...
      ".emacs.d/init.el".text = ''
        ...
        (provide 'init)
      '';
      ".emacs.d/hm-early-init.el" = {
        source = "${config.programs.emacs.finalPackage.deps}/share/emacs/site-lisp/hm-early-init.el";
      };
      ".emacs.d/hm-init.el" = {
        source = "${config.programs.emacs.finalPackage.deps}/share/emacs/site-lisp/hm-init.el";
      };
    };
  };
}
